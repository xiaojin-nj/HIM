...
def add_summary_gradient(dnn_optimizer,loss,scop_gra=['dnn_item', 'dnn_user', 'dnn_seq', 'seq_to_user']):
    for i in range(len(scop_gra)):
        gradients = dnn_optimizer.compute_gradients(loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scop_gra[i]))
        for g, v in gradients:
            tf.summary.scalar("optimization/%s_gradient_norm" % v.name, tf.norm(g))
            tf.summary.scalar("optimization/%s_gradient_max" % v.name, tf.reduce_max(g))
            tf.summary.scalar("optimization/%s_gradient_min" % v.name, tf.reduce_min(g))
